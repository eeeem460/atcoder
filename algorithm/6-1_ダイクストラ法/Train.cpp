...
struct edge {
    ...
    edge(ll to, ll tr, ll w) : to(to), tr(tr), w(w) {
    }
...
int main() {
    ...
    for (ll i = 0; i < m; i++) {
        cin >> a[i] >> b[i] >> t[i] >> k[i];
        a[i]--, b[i]--;
        g[a[i]].push_back(edge(b[i], i, t[i]));
        g[b[i]].push_back(edge(a[i], i, t[i]));
    }

    vector<ll> fastest(n, LINF);
    fastest[x] = 0;
    priority_queue<pair<ll, ll>, vector<pair<ll, ll>>, greater<pair<ll, ll>>> qu;
    qu.push({0, x});

    while (qu.size()) {
        auto [now_time, now_town] = qu.top();
        qu.pop();

        // 注意！！！！ 既に見た頂点のcontinue https://snuke.hatenablog.com/entry/2021/02/22/102734
        if (fastest[now_town] < now_time) continue;

        for (auto&& [next, train_index, t] : g[now_town]) {
            ll next_time = (now_time + k[train_index] - 1) / k[train_index] * k[train_index] + t;

            if (chmin(fastest[next], next_time)) {
                qu.push({next_time, next});
            }
        }
    }
