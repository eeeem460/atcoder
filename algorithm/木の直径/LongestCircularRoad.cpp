...
int main() {
    ...
    auto dfs = [&](auto&& self, ll prv_e, ll now_e, ll now_d) -> pair<ll, ll> {
        ll res_e = now_e, res_d = now_d;

        if (g[now_e] == vector<ll>{prv_e}) return {res_e, res_d};

        for (auto&& next : g[now_e]) {
            if (next == prv_e) continue;

            auto [tmp_e, tmp_d] = self(self, now_e, next, now_d + 1);
            if (chmax(res_d, tmp_d)) {
                res_e = tmp_e;
            }
        }
    ...
    auto longest_e = dfs(dfs, -1, 0, 0).first;

    cout << dfs(dfs, -1, longest_e, 0).second + 1 << endl;
}
