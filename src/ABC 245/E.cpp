...
int main() {
    ...
    auto dfs = [&](auto&& self, ll now, vector<bool>& now_roop) -> bool {  // self(self, );
        if (now_roop[now] == true) return true;
        if (roop[now] == true) return true;
        if (seen[now]) return roop[now];
