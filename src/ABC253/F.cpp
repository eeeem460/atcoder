...
int main() {
    ...
    while (q--) {
        ...
        if (Q == 1) {
            ...
            auto right = vmp[now].lower_bound(r);
            auto l_right = vmp[now].lower_bound(l);
            ll right_v = right->second;
            ll l_right_v = l_right->second;

            vmp[now][r] = right_v + x;
            if (right_v == l_right_v and vmp[now][l - 1] > 0) {
                vmp[now][l - 1] = right_v;
            }
            else {
                vmp[now][l - 1] = 
