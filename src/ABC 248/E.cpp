...
int main() {
    ...
    set<pair<Bint, pair<Bint, Bint>>> st;
    ll ans = 0;
    for (ll i = 0; i < n; i++) {
        for (ll j = i + 1; j < n; j++) {
            ll ax = x[i], ay = y[i];
            ...
            for (ll k = j + 1; k < n; k++) {
                ...
                if ((Bint)(y[k] - ax) * (bx - ax) == (Bint)(by - ay) * (x[k] - ax)) cnt++;
            }

            if (cnt >= k) {
                Bint t1 = bx - ax;
                Bint t2 = by - ay;
                Bint t3 = (Bint)(-ax) * (by - ay) + (Bint)ay * (bx - ax);

                // ll G12 = abs(gcd(t1, t2));
                // ll G23 = abs(gcd(t2, t3));
                // ll G31 = abs(gcd(t3, t1));
                // if (G12 == 0 and ) {
                //     t1
                // }

                Bint G = Bgcd(t3, Bgcd(t2, t1));
                if (G < 0) G *= -1;
                t1 /= G, t2 /= G, t3 /= G;
