...
int main() {
    ...
    while (q--) {
        ...
        auto itrl = lower_bound(cnt[x].begin(), cnt[x].end(), l);
        auto itrr = lower_bound(cnt[x].begin(), cnt[x].end(), r);

        // ll ans = (cnt[x].end() - itrl) - (cnt[x].end() - itrr) - 1;
        ll ans = itrr - itrl;

        // if (itrl != cnt[x].end() and (*itrl == l)) ans++;
        if ((itrr != cnt[x].end()) and (*itrr == r)) ans++;
