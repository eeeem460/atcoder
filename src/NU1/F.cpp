...
int main() {
    ...
    auto dfs = [&](auto&& self, ll st, ll now_e, ll now_c, vector<ll>& seen) -> bool {  // self(self, );
        if (now_e == st and seen[now_e] != -LINF) {
            if (seen[now_e] > 0)
                return true;
            else
                return false;
        ...
        bool can = false;
        for (auto&& x : g[now_e]) {
            can |= self(self, st, x.to, now_c + x.w, seen);
        }
    ...
    vector<bool> p_roop(n);
    for (ll i = 0; i < n; i++) {
        vector<ll> sn(n, -LINF);
        if (dfs(dfs, i, i, 0, sn)) {
            if (sn[n - 1]) p_roop[i] = true;
        }
    }

    vector<ll> sn_0(n, -LINF);
    dfs(dfs, 0, 0, 0, sn_0);
    for (ll i = 0; i < n; i++) {
        if (p_roop[i] and sn_0[i]) {
            cout << -1 << endl;
            return 0;
        }
    }
