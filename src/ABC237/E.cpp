...
int main() {
    ...
    for (ll i = 0; i < m; i++) {
        ...
        u--, v--;
        if (h[u] > h[v]) {
            g[u].push_back(edge(v, h[u] - h[v]));
            g[v].push_back(edge(u, 2 * (h[v] - h[u])));
        }
        else {
            g[u].push_back(edge(v, 2 * (h[u] - h[v])));
            g[v].push_back(edge(u, (h[v] - h[u])));
        }
    }
    ...
    while (qu.size()) {
        auto [now_joy, now] = qu.top();
        qu.pop();

        for (auto&& e : g[now]) {
            ll to = e.to, w = e.w;
            if (chmax(joy[to], joy[now] + w)) {
                qu.push({joy[to], to});
            }
        }
    }
