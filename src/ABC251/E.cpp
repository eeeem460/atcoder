...
int main() {
    ...
    for (ll i = 1; i < n; i++) {
        ...
        if (i + 2 < n) {
            chmin(dp[0][i + 2], dp[0][i] + a[imn]);
        }
        if (i + 1 < n) chmin(dp[1][i + 1], dp[0][i] + a[im]);

        if (i + 2 < n) chmin(dp[0][i + 2], dp[1][i] + a[imn]);
        if (i + 1 < n) chmin(dp[0][i + 1], dp[1][i] + a[im] - a[imb]);
        if (i + 1 < n) chmin(dp[1][i + 1], dp[1][i] + a[im]);
    }

    ll ans = min(dp[0][n - 1], dp[1][n - 1]);

    vector dp2(2, vector<ll>(n, LINF));
    dp2[0][0] = a[n - 1];
    for (ll i = 0; i < n; i++) {
        ll im = i % n;
        ll imb = (n + i - 1) % n;
        ...
        if (i + 2 < n) chmin(dp2[0][i + 2], dp2[0][i] + a[imn]);
        if (i + 1 < n) chmin(dp2[1][i + 1], dp2[0][i] + a[im]);

        if (i + 2 < n) chmin(dp2[0][i + 2], dp2[1][i] + a[imn]);
        if (i + 1 < n) chmin(dp2[0][i + 1], dp2[1][i] + a[im] - a[imb]);
        if (i + 1 < n) chmin(dp2[1][i + 1], dp2[1][i] + a[im]);
    }

    ans = min(ans, min({dp2[0][n - 2], dp2[1][n - 2], dp2[0][n - 1], dp2[1][n - 1]}));
